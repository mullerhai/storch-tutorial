...
class MODEL(nn.Module):
    ...
    def __init__(self, n_question, batch_size, q_embed_dim, qa_embed_dim, memory_size, final_fc_dim):
        ...
        self.beta_linear = nn.Linear(self.q_embed_dim, 1, bias=True)

        self.init_memory_key = nn.Parameter(torch.randn(self.memory_size, self.memory_key_state_dim))  # Mk的emb
        nn.init.kaiming_normal_(self.init_memory_key)
        self.init_memory_value = nn.Parameter(torch.randn(self.memory_size, self.memory_value_state_dim))  # Ntv的emb
        nn.init.kaiming_normal_(self.init_memory_value)

        self.mem = DeepIRT(memory_size=self.memory_size,
    ...
    def forward(self, q_data, qa_data, target):
        ...
        q_embed_content = torch.cat([q_embed_l[i].unsqueeze(1) for i in range(seqlen)], 1).view(batch_size * seqlen, -1)
        ...
        seta = self.predict_linear(read_content_embed)
        beta = torch.tanh(self.beta_linear(q_embed_content))
        pred = torch.tanh(0.3*seta - beta)
