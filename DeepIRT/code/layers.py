...
class DeepIRTLayer(nn.Module):
    def __init__(self, memory_size, memory_state_dim, is_write):
        super(DeepIRTLayer, self).__init__()
        """"
        Parameters
            memory_size:        scalar
    ...
    def write(self, control_input, memory, write_weight):
        ...
        erase_signal = torch.sigmoid(self.erase(control_input))  # et
        add_signal = torch.tanh(self.add(control_input))   # at
...
class DeepIRT(nn.Module):
    def __init__(self, memory_size, memory_key_state_dim, memory_value_state_dim, init_memory_key):
        ...
        self.key_head = DeepIRTLayer(memory_size=self.memory_size,
                                       memory_state_dim=self.memory_key_state_dim,
                                       is_write=False)

        self.value_head = DeepIRTLayer(memory_size=self.memory_size,
