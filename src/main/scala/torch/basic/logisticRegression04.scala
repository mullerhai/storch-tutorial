...
import org.bytedeco.pytorch.{ChunkDatasetOptions, Example, ExampleIterator, 
  ExampleStack, ExampleVector, ExampleVectorIterator, 
  JavaDataset, JavaDistributedRandomTensorDataLoader, 
  JavaDistributedSequentialTensorDataLoader,
  JavaRandomDataLoader, JavaRandomTensorDataLoader, 
  JavaSequentialTensorDataLoader, JavaStatefulDataset, JavaStreamDataLoader,
  RandomSampler, SizeTArrayRef, SizeTOptional, TensorExample, TensorExampleIterator, 
  TensorExampleStack, TensorExampleVector,
//  ChunkDataReader as CDR, ChunkDataset as CD, ChunkRandomDataLoader as CRDL, 
//  ChunkSharedBatchDataset as CSBD, DataLoaderOptions as DLO, DistributedRandomSampler as DRS, 
//  DistributedSequentialSampler as DSS, JavaStreamDataset as JSD, JavaTensorDataset as TD, 
//  StreamSampler as STS
}
...
import torch.utils.data.dataset.normal.stream.{StreamDataset, StreamTensorDataset}
import torch.utils.data.dataset.normal.{StatefulDataset, StatefulTensorDataset, JavaDataset as JD}
import torch.utils.data.sampler.distribute.{DistributedRandomSampler, DistributedSequentialSampler}
import torch.utils.data.sampler.stream.StreamSampler
import torch.utils.data.sampler.{RandomSampler as RS, SequentialSampler as SS}
import torch.utils.data.{DataLoaderOptions, TensorDataset}


object logisticRegression04 extends App {
...
//    val model = nn.Linear(input_size, num_classes)
